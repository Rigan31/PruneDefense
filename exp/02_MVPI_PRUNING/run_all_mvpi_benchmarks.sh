...
LOG_ROOT="${ROOT}/log/13_MVPI_PRUNING"
mkdir -p "${LOG_ROOT}"
...
# ── All datasets ───────────────────────────────────────────────────────────
# Format: "DATASET_PATH|SHORT_NAME|OBJECT_LIST_VAR"
#   SHORT_NAME is used for the log folder: log/13_MVPI_PRUNING/<SHORT_NAME>_<object>/
DATASETS=(
...
log "======================================================"
log "  13_MVPI_PRUNING -- Full Benchmark"
log "  Started : $(date)"
log "  GPU     : ${GPU}"
log "  Log     : ${MAIN_LOG}"
log "======================================================"
...
% Auto-generated from 13_MVPI_PRUNING benchmark.
% \input{results_table.tex}
% Requires: \usepackage{booktabs}

\begin{table*}[t]
\centering
\caption{MVPI defense benchmark across all datasets and scenes. Gaussians = peak count during training (millions). Time = total training time (minutes). GPU = peak GPU memory (MB). FPS = render speed (frames per second). SSIM and PSNR are evaluated on training views.}
